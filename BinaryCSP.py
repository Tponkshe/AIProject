			...
			if consistent(assignment, csp, var, val):
				...
				if result != None:
					...
					for var2, val2 in inferences:
						assignment.varDomains[var2].add(val2)  # assign values in inferences
	...
	for i in range(0, len(vars)):
		if((assignment.assignedValues[vars[i]] == None)):
			if((minDomain > len(domains[vars[i]]))):
				...
				varsWithMinDomain.append(vars[i])
			elif((minDomain == len(domains[vars[i]]))):
				varsWithMinDomain.append(vars[i])
	if(len(varsWithMinDomain) == 0):
		return None
	elif(len(varsWithMinDomain) == 1):
		return varsWithMinDomain[0]
	else:
		count = 0
		maxDegree = 0
		returnVar = None
		for var in varsWithMinDomain:
			count = 0
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for x in csp.binaryConstraints:
		if(x.affects(var)):
			dependedVars.append(x.otherVariable(var))
	for value in values:
		count = 0
		for var1 in dependedVars:
			if(value in list(assignment.varDomains[var1])):
				count = count + 1
		heuristicVals.append((value, count))
	sorted_by_second = sorted(heuristicVals, key=lambda tup: tup[1])
...
def forwardChecking(assignment, csp, var, value):
	...
	for variable2 in dependents:		
		if(value in list(domains[variable2])):
			if (len(domains[variable2]) == 1):
				for v in seenVars:
					assignment.varDomains[v].add(value)
				return None
			else:
				domains[variable2].remove(value)
				inferences.add((variable2, value))
				seenVars.append(variable2)
# 	for vars1 in seenVars:
# 		assignment.varDomains[vars1].remove(value)
# 		inferences.add((vars1, value))						
	return inferences
...
# 				print((var2, val2))
			...
			if(counter == len(assignment.varDomains[var2])):
				return None
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if (constraint.affects(var) and assignment.assignedValues[constraint.otherVariable(var)] == None):
 			iDepend.append(constraint.otherVariable(var))
			...
			if(len(inference2) != 0):
				for constraint in csp.binaryConstraints:
					if (constraint.affects(tuple[1]) and assignment.assignedValues[constraint.otherVariable(tuple[1])] == None):
						t = (tuple[1], constraint.otherVariable(tuple[1]), constraint)
						q.put(t)
...
def AC3(assignment, csp):
	...
	for variable in csp.varDomains.keys():
		for constraint in csp.binaryConstraints:
			if (constraint.affects(variable)):
	 			iDepend.append(constraint.otherVariable(variable))
				t = (variable, constraint.otherVariable(variable), constraint)
				myQueue.put(t)
	while(myQueue.empty() == False):
		...
		if(inference2 == None):
			...
			if(len(inference2) != 0):
				for constraint in csp.binaryConstraints:
					if (constraint.affects(dequed[1]) and assignment.assignedValues[constraint.otherVariable(dequed[1])] == None):
						t = (dequed[1], constraint.otherVariable(dequed[1]), constraint)
						myQueue.put(t)
				inferences = inferences.union(inference2)
	return assignment
